...
class BilibiliClient:
    ...
    def _on_request(self, request: httpx.Request):
        ...
        if request.url.path == "/api/ticket/order/createV2":
            request.headers.update({
                "x-risk-header": self.risk_header
            })
        else:
            request.headers.pop("x-risk-header", None)
        pass
    ...
    def generate_ctoken(
        self,
        touchend=random.randint(1, 30),
        visibilitychange=random.randint(0, 3),
        openWindow=random.randint(0, 3),
    ...
    def generate_ptoken(self,ctoken, uid, timestamp):
        ...
        token_bytes = b"\x00"
        temp_byte = ctoken_bytes[0]+ctoken_bytes[1]
        temp_byte %= 256
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = timestamp//100000000%100
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = ctoken_bytes[4]+ctoken_bytes[5]
        temp_byte %= 256
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = timestamp//10000000%10
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = timestamp//1000000%10
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = ctoken_bytes[10]+ctoken_bytes[11]
        temp_byte %= 256
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        token_bytes += (uid%100).to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = timestamp//100000%10
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = ctoken_bytes[16]+ctoken_bytes[17]
        temp_byte %= 256
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = ctoken_bytes[18]+ctoken_bytes[19]
        temp_byte %= 256
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = ctoken_bytes[20]+ctoken_bytes[21]
        temp_byte %= 256
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = ctoken_bytes[22]+ctoken_bytes[23]
        temp_byte %= 256
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = timestamp//10000%10
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = timestamp//1000%10
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = timestamp//100%10
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        token_bytes += b"\x00"
        temp_byte = timestamp%100
        token_bytes += temp_byte.to_bytes(1, byteorder='big')
        return base64.b64encode(token_bytes).decode('utf-8')
    ...
    def save(self):
        ...
        logger.debug(self._cookies)
        logger.debug("Origin cookies:")
